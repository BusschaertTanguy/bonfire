...
namespace Presentation.Sessions.Routes;

internal static class SessionRoutes
{
    extension(IEndpointRouteBuilder app)
    {
        internal void MapSessionRoutes()
        {
            ...
            {
                var result =
                    await authorizationService.AuthorizeAsync(httpContext.User, id, Policies.SessionParticipant);
                if (!result.Succeeded)
                {
